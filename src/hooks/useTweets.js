  ...
  const preloadAll = useCallback(async () => {
    ...
    for (const days of periods) {
      ...
      const [authRes, tweetsRes] = await Promise.all([
        fetch(`https://api-conflit-twitter.ovh/api/twitter_conflicts/authors?start_date=${start}&end_date=${end}`).catch(() => null),
        fetch(`https://api-conflit-twitter.ovh/api/twitter_conflicts/tweets.geojson?start_date=${start}&end_date=${end}`).catch(() => null),
      ]);
      if (authRes?.ok) {
