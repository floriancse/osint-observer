    ...
    map.on('load', () => {
      ...
      for (let x = 0; x < size; x++) {
        for (let y = 0; y < size; y++) {
          const i = (y * size + x) * 4;
          if ((x + y) % 8 < 2) {
            hatchImage[i] = 136; hatchImage[i+1] = 0;
            hatchImage[i+2] = 0; hatchImage[i+3] = 255;
          }
        }
      }
      ...
      // Layers disputed
      map.addLayer({ id: 'disputed_area_fill', type: 'fill', source: 'disputed_area',
        paint: { 'fill-pattern': 'hatch-pattern', 'fill-opacity': 0.5 } });
      map.addLayer({ id: 'disputed_area_outline', type: 'line', source: 'disputed_area',
        paint: { 'line-color': '#880000', 'line-width': 1, 'line-opacity': 1 } });

      // Layers world areas
      map.addLayer({ id: 'world_areas_fill', type: 'fill', source: 'world_areas',
